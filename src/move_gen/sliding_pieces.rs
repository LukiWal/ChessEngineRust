...
pub fn is_attacked_by_bishop(board : &Board, square : Square, attacking_color : Color) -> bool{  
    is_attacked_by_sliding_piece(&board, square, attacking_color, BISHOP_OFFSETS, PieceType::Rook)
}

pub fn is_attacked_by_queen(board : &Board, square : Square, attacking_color : Color) -> bool{  
    is_attacked_by_sliding_piece(&board, square, attacking_color, QUEEN_OFFSETS, PieceType::Rook)
}
    ...
    for (row_direction, col_direction) in offsets{
        ...
        while let Some(attacking_square) = Square::new(row, col){
            if board.is_occupied(attacking_square){
                if board.get_piece(attacking_square).is_some_and(|attacking_piece| attacking_piece.piece_type == attacking_piece_type) {return true;}
                break;
            }
            row += row_direction;
            col += col_direction;
